...
def new_article():
    if request.method == 'POST':
        ...
        postId = uuid.uuid1()
        try:
            db.execute(db.prepare("INSERT INTO articles(id, posted, title, content, author) VALUES(?, toTimestamp(now()), ?,?,?);"), [postId, request.get_json()['title'], request.get_json()['content'], user])
...
def edit_article(article_id):
    if request.method == 'POST':
        ...
        user=auth.getUser()
        db = database.get_db(SERVICE_NAME)
        for row in db.execute(db.prepare("SELECT id FROM articles WHERE id=? AND author=?;"), [article_id, user]):
            if row != None:
                db.execute(db.prepare("UPDATE articles SET content=?, posted=CURRENT_TIMESTAMP WHERE id=? AND author=?;"), [request.get_json()['content'], article_id, user])
