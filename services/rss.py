    ...
    if flask_request.method == 'GET':
        ...
        if response.status_code == requests.codes.ok:
            ...
            for article in articles:
                ...
                articleItem.link = f'https://localhost:5100/article/{article_id}'
                response = requests.get(f'http://localhost:5100/article/{article_id}')
                if response.status_code == requests.codes.ok:
                    ...
                    articleItem.pubDate= datetime.datetime.strptime(articleInfo['posted'], "%Y-%m-%d %H:%M:%S.%f")
                response = requests.get(f'http://localhost:5200/article/{article_id}/tags')
                if response.status_code == requests.codes.ok:
                    articleItem.categories = response.json()['tags']
                response = requests.get(f'http://localhost:5300/article/{article_id}/comments')
                if response.status_code == requests.codes.ok:
                    articleItem.comments = response.json()['count']
                article_collection.append(articleItem)
            feed = Feed(
                title = "New Kids On The Blog RSS Feed",
                link = f'http://localhost:5100/article/{article_id}',
                description = "RSS 2.0 feed generated with rfeed",
                language = "en-US",
                items = article_collection
            )
...
def comment_articles():
    if flask_request.method == 'GET':
        ...
        if response.status_code == requests.codes.ok:
            articles = response.json()['success']
            for article in articles:
                response = requests.get(f'http://localhost:5300/article/{article_id}/comments/100')
                print(response.json())
